...
<div class="row">
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header"><i class="fa fa-table"></i> Team Leaders View</div>

            <div class="card-body">
                {{-- <div class="float-right mb-3">
                    <button class="btn btn-success" data-toggle="modal" data-target="#modal-team-insert">
                        New Team Leader
                    </button>
                </div> --}}
                <div class="clearfix"></div>
                <input type="hidden" id="tid" value="{{ $role_id }}">
                    ...
                    <table id="classTable" class="table table-bordered" style="width: 100% !important;">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                {{-- <td> <img src="{{asset($member->NID)}}" style="width:50px;height:50px"
                                ...
                                <td>
                                    ...
                                    {{-- <a href="javascript:void(0)" onclick="deleteTeamLeader({{$member->user->id}})"
                                        style="padding: 5px 10px;" class="btn btn-default btn-xs border"
...
{{-- modal delivery team leader history --}}
...
<div class="modal fade" id="modal-member-update" style="display: none;" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content animated flipInX">
            <div class="modal-header" style="border-bottom: none;">
                <h4 class="modal-title" style="font-size: 18px;">Update Team Leader Info</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                </button>
            </div>
            <div class="modal-body">
                <form id="teamUpdateForm">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="email" placeholder="contact number" name="email"
                            required>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="district" placeholder="contact number"
                            name="district" required>
                    </div>

                    <div class="form-group">
                        <label for="input-1">City</label>
                        <input type="text" class="form-control" id="city" placeholder="contact number" name="city"
                            required>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="house_no" placeholder="contact number"
...
<script>
    ...
    /**
     * @name getTeamLeaderDeliveryHistoryAjax
    ...
    function viewHistory(id) {
        $.ajax({
            url: '{{ URL("getTeamLeaderDeliveryHistoryAjax") }}',
            type: 'POST',
            data: {
                id: id
            },
            success: data => {
                ...
                $.each(data, (key, val) => {
                    historyView += `
                        <tr>
                            <td>${++count}</td>
                            <td>${String(val.id).padStart(4, '0')}</td>
                            <td>${val.first_name} ${val.last_name}</td>
                            <td>${val.area}, ${val.city}</td>
                            <td>${val.shipment_assigned_at ? val.shipment_assigned_at : ''}</td>
                            <td>${val.shipment ? val.shipment.deadline_date ? val.shipment.deadline_date : '' : ''}</td>
                            <td>${val.shipment ? val.shipment.deadline_time ? val.shipment.deadline_time : '' : ''}</td>
                            <td>${val.shipment ? val.shipment.completed_at ? val.shipment.completed_at : '' : ''}</td>
                        </tr>
                    `;
                });
     ...
     *
     */
    function deleteTeamLeader(id) {
        alertify.confirm("Are You Sure To Delete This?",
            function () {
                $.ajax({
                    type: 'post',
                    url: '{{URL("teamLeaderDeleteAjax")}}',
                    data: {
                        id: id
                    },
                    dataType: 'json',
