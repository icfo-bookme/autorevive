...
@section('content')
    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header"><i class="fa fa-table"></i> Cost Sub Category View</div>
                <div class="card-body">
    ...
    <div class="modal fade" id="modal-subcategory-insert" style="display: none;" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content animated flipInX">
                ...
                <form id="costSubCategoryInsertForm" method="post">
                    @csrf
    ...
    <script>
        $(document).ready(function() {
            ...
            var dataTable = $('#subCategoryTable').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: route('listAllCostSubCategories'),
                    data: function(data) {
             ...
             */
            $('#costSubCategoryInsertForm').submit(function(event) {
                event.preventDefault();

                alertify.confirm('Are You Sure ?', 'Data Will Be Inserted', function() {
                    $.ajax({
                        type: 'post',
                        url: '{{ URl('costSubCategoryInsert') }}',
                        data: $('#costSubCategoryInsertForm').serialize(),
                        dataType: 'json',
                        success: function(response) {
                            if (response.status === true) {
                                alertify.success(response.message);
                                dataTable.ajax.reload();
                                $('#costSubCategoryInsertForm')[0].reset();
        ...
        /**
         * Fetch cost sub category edit form and update
         * @param id
         */
        function costSubCategoryEdit(id) {
            $.ajax({
                type: 'post',
                url: '{{URL("getCostSubCategoryEditForm")}}',
                data: {
                    id: id
                },
                dataType: 'json',
                success: function (response) {
                    if(response.status){
                        ...
                        $('#costSubCategoryUpdateForm').submit(function (event) {
                            event.preventDefault();
                            alertify.confirm('Are You Sure ?', 'Data Will Be Updated', function () {
                                $.ajax({
                                    type: 'post',
                                    url: '{{URl("costSubCategoryUpdate")}}',
                                    data: $('#costSubCategoryUpdateForm').serialize(),
                                    dataType: 'json',
                                    success: function (response) {
                                        if (response.status === true) {
                                            alertify.success(response.message);
                                            ...
                                            $('#costSubCategoryUpdateForm')[0].reset();
                                        } else if (response.status === false) {
                                            alertify.error("<span class='text-white'>"+response.message+"</span>");
                                        } else if (response.status === "validation-error") {
                                            for (let key in response.data) {
        ...
        /**
         * Soft deletes cost sub category
         * @param id
         */
        function costSubCategoryDelete(id) {
            alertify.confirm("Are You Sure To Delete This?",
                function () {
                    $.ajax({
                        type: 'post',
                        url: '{{URL("costSubCategoryDelete")}}',
                        data: {
                            id: id
                        },
                        dataType: 'json',
