...
@section('content')
    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header"><i class="fa fa-table"></i> Fund Category View</div>
                <div class="card-body">
                    <div class="float-right mb-3">
                        <button class="btn btn-success" data-toggle="modal" data-target="#modal-category-insert">New Category</button>
                    </div>
    ...
    <div class="modal fade" id="modal-category-insert" style="display: none;" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content animated flipInX">
                ...
                <form id="fundCategoryInsertForm" method="post">
                    @csrf
    ...
    <script>
        $(document).ready(function () {
            ...
            var dataTable = $('#categoryTable').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: route('listAllFundCategories'),
                    data: function (data) {
            ...
            /**
             * Inserts fund category
             */
            $('#fundCategoryInsertForm').submit(function (event) {
                event.preventDefault();

                alertify.confirm('Are You Sure ?', 'Data Will Be Inserted', function () {
                    $.ajax({
                        type: 'post',
                        url: '{{URl("fundCategoryInsert")}}',
                        data: $('#fundCategoryInsertForm').serialize(),
                        dataType: 'json',
                        success: function (response) {
                            if(response.status === true){
                                alertify.success(response.message);
                                dataTable.ajax.reload();
                                $('#fundCategoryInsertForm')[0].reset();
        ...
        /**
         * Fetch fund category edit form and update
         * @param id
         */
        function fundCategoryEdit(id) {
            $.ajax({
                type: 'post',
                url: '{{URL("getFundCategoryEditForm")}}',
                data: {
                    id: id
                },
                dataType: 'json',
                success: function (response) {
                    if(response.status){
                        ...
                        $('#fundCategoryUpdateForm').submit(function (event) {
                            event.preventDefault();
                            alertify.confirm('Are You Sure ?', 'Data Will Be Updated', function () {
                                $.ajax({
                                    type: 'post',
                                    url: '{{URl("fundCategoryUpdate")}}',
                                    data: $('#fundCategoryUpdateForm').serialize(),
                                    dataType: 'json',
                                    success: function (response) {
                                        if (response.status === true) {
                                            alertify.success(response.message);
                                            ...
                                            $('#fundCategoryUpdateForm')[0].reset();
                                        } else if (response.status === false) {
                                            alertify.error("<span class='text-white'>"+response.message+"</span>");
                                        } else if (response.status === "validation-error") {
                                            for (let key in response.data) {
        ...
        /**
         * Soft deletes fund category
         * @param id
         */
        function fundCategoryDelete(id) {
            alertify.confirm("Are You Sure To Delete This?",
                function () {
                    $.ajax({
                        type: 'post',
                        url: '{{URL("fundCategoryDelete")}}',
                        data: {
                            id: id
                        },
                        dataType: 'json',
